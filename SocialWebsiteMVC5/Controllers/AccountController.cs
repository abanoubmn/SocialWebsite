...
using SocialWebsiteMVC5.Models;
...
namespace SocialWebsiteMVC5.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                var account = db.Accounts.First(a => a.UserName == model.UserName);
                if (account != null)
                {
        ...
        public ActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var account = db.Accounts.Where(a => a.UserName == model.UserName);
                    if (account.Count()==0)
                    {
                        ...
                        db.Accounts.Add(regAccount);
                        db.SaveChanges();
                        SignIn(regAccount, true);
                        return RedirectToAction("Index", "Home");
                    }
                    else
                    {
                        ViewBag.UsernameTaken = "Username is taken, try another one!";
                        //AddErrors(result);
                    }
                }
                catch (Exception)
